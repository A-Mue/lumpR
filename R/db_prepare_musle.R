...
#' 
#' @param copy_from_other_tables Copy values to table "soil_veg_components": MUSLE-C and Manning-n from table "vegetation", MUSLE-K from "soils", coarse_frac copies coarse_frag from table "horizons" topmost soil horizon 1 . Array of strings with possible values \code{c("MUSLE-C", "Manning-n" ,"MUSLE-K","coarse_frac")} of
#' \code{\link[lumpR]{db_create}}.
#' 
...
db_prepare_musle <- function(
  ...
  {  
    statement =  "UPDATE soil_veg_components INNER JOIN vegetation ON vegetation.pid=soil_veg_components.veg_id SET musle_c1 = c_musle_c1;"
    res = sqlQuery2(con, statement, info="copy MUSLE-C1") 
    statement =  "UPDATE soil_veg_components INNER JOIN vegetation ON vegetation.pid=soil_veg_components.veg_id SET musle_c2 = c_musle_c2;"
    res = sqlQuery2(con, statement, info="copy MUSLE-C2")     
    statement =  "UPDATE soil_veg_components INNER JOIN vegetation ON vegetation.pid=soil_veg_components.veg_id SET musle_c3 = c_musle_c3;"
    res = sqlQuery2(con, statement, info="copy MUSLE-C3")     
    statement =  "UPDATE soil_veg_components INNER JOIN vegetation ON vegetation.pid=soil_veg_components.veg_id SET musle_c4 = c_musle_c4;"
    res = sqlQuery2(con, statement, info="copy MUSLE-C4")  
  }
  
  if ("coarse_frac-C" %in% toupper(copy_from_other_tables))
  {  
    statement =  "UPDATE soil_veg_components INNER JOIN horizons ON horizons.soil_id=soil_veg_components.soil_id SET coarse_frac = coarse_frag WHERE position = 1;"
